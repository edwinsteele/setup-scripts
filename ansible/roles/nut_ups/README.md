...
Diversity DVB tuner (`roles/dvb_tuner`) - `lsusb -t` shows both under
`Bus 003 Device 002`. Early on 2026-08-15, with that hub unpowered, its
downstream ports were running off whatever the host's USB2 port budget
left over, and `journalctl -k` showed the UPS's USB link cleanly
...
that often would have been worse than not monitoring at all, since the one
moment this needs to actually work is mid-outage - hence this role
initially shipped with `nut_ups_enabled: false`.

The hub has since been given proper power (its power-input port is rated
5V/2.1A - any real USB charger/wall brick meeting that is fine; just not a
computer's USB port, which can drop power on sleep), confirmed via
`journalctl -k`: no `USB disconnect` lines for `0764:0501` since shortly
after that boot. The gate has been removed and the role now runs
unconditionally. If the hub ever loses power again and the link starts
flapping, re-check with:
...
ansible-playbook -i inventory.yml site.yml --tags nut_ups \
  --limit viking.home.wordspeak.org
```
